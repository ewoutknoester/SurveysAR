...
#TODO: check & deal with outlier HC model
...
# Set order of genera
df3.HC.genus.avg$Genus.o <- factor(df3.HC.genus.avg$Genus.o,  levels=c("Acropora", "Pocillopora", "Porites", "Stylophora",   "Dipsastraea", "Echinopora", "Favites", "Galaxea", "Goniastrea", "Hydnophora", "Porites (massive)", "Other"))
...
# Hard coral
hist(df3.BC.hc$Cover)

# Soft coral
hist(df3.BC.sc$Cover)

# Macroalgae
hist(df3.BC.ma$Cover)

# Hard substrate
hist(df3.BC.hs$Cover)

# Soft substrate
hist(df3.BC.ss$Cover)
...
## Model
HC.bm1 <- betareg(Cover ~ Treatment*Location, data = df3.BC.hc.mod)

### Allowing for variable precision
HC.bm1b <- betareg(Cover ~ Treatment*Location | Treatment, data = df3.BC.hc.mod)
HC.bm1c <- betareg(Cover ~ Treatment*Location | Location, data = df3.BC.hc.mod)
HC.bm1d <- betareg(Cover ~ Treatment*Location | Treatment*Location, data = df3.BC.hc.mod)

AIC(HC.bm1, HC.bm1b, HC.bm1c, HC.bm1d)

## Model selection fixed part
HC.bm2T <- betareg(Cover ~ Treatment | Treatment*Location, data = df3.BC.hc.mod)
HC.bm2TL <- betareg(Cover ~ Treatment*Location | Treatment*Location, data = df3.BC.hc.mod)
...
## Betareg full model
SC.bm1 <- betareg(Cover ~ Treatment*Location, data = df3.BC.hc.mod) # Variable precision did not improve residuals

### Allowing for variable precision (choices based on residual plots)
SC.bm1b <- betareg(Cover ~ Treatment*Location | Treatment, data = df3.BC.sc.mod)
SC.bm1c <- betareg(Cover ~ Treatment*Location | Location, data = df3.BC.sc.mod)
SC.bm1d <- betareg(Cover ~ Treatment*Location | Treatment*Location, data = df3.BC.sc.mod)
...
## Betareg full model
MA.bm1 <- betareg(Cover ~ Treatment*Location, data = df3.BC.ma.mod) # Variable precision did not improve residuals

### Allowing for variable precision (choices based on residual plots)
MA.bm1b <- betareg(Cover ~ Treatment*Location | Treatment, data = df3.BC.ma.mod)
MA.bm1c <- betareg(Cover ~ Treatment*Location | Location, data = df3.BC.ma.mod)
MA.bm1d <- betareg(Cover ~ Treatment*Location | Treatment*Location, data = df3.BC.ma.mod)
...
## Betareg full model
HS.bm1 <- betareg(Cover ~ Treatment*Location, data = df3.BC.hs.mod) # Variable precision did not improve residuals

### Allowing for variable precision (choices based on residual plots)
HS.bm1b <- betareg(Cover ~ Treatment*Location | Treatment, data = df3.BC.hs.mod)
HS.bm1c <- betareg(Cover ~ Treatment*Location | Location, data = df3.BC.hs.mod)
HS.bm1d <- betareg(Cover ~ Treatment*Location | Treatment*Location, data = df3.BC.hs.mod)
...
## Betareg full model
SS.bm1 <- betareg(Cover ~ Treatment*Location, data = df3.BC.ss.mod) # Variable precision did not improve residuals

### Allowing for variable precision (choices based on residual plots)
SS.bm1b <- betareg(Cover ~ Treatment*Location | Treatment, data = df3.BC.ss.mod)
SS.bm1c <- betareg(Cover ~ Treatment*Location | Location, data = df3.BC.ss.mod)
SS.bm1d <- betareg(Cover ~ Treatment*Location | Treatment*Location, data = df3.BC.ss.mod)
...
# Treatments in 2022
ggplot(df3.BC.22, aes(fill=Substrate, y=Cover, x=Treatment)) + 
    geom_bar(position="fill", stat="identity")+
    scale_fill_manual(values = c("#EAEDED", "#909497", "#424949", "#229954", "#F9E79F", "#E67E22"))+
    labs(y = "Benthic cover (%)", x = "Treatment", fill = "Substrate")+
    scale_y_continuous(expand = c(0,0))+
...
ggsave("Benthic cover_Benthic cover 2022.tiff", width = 23, height = 12, units = "cm", dpi=1200, compression = "lzw")

# Treatments in 2020
ggplot(df3.BC.20, aes(fill=Substrate, y=Cover, x=Treatment)) + 
    geom_bar(position="fill", stat="identity")+
    scale_fill_manual(values = c("#EAEDED", "#909497", "#424949", "#229954", "#F9E79F", "#E67E22"))+
    labs(y = "Benthic cover (%)", x = "Treatment", fill = "Substrate")+
    scale_y_continuous(expand = c(0,0))+
...
ggsave("Benthic cover_Benthic cover 2020.tiff", width = 23, height = 12, units = "cm", dpi=1200, compression = "lzw")

# HARD CORAL BY GENUS
## Legend
# Create dataframe for legend    
dt <- data.table(x = 1, y = seq(1, 12, 1), z = factor(1:12))
dt[ , grp := cut(as.numeric(z), breaks = c(0,8,12),
...
HC.SE <- ggplot(df3.HC.sum) +  
  ...
  scale_fill_manual("Genus.o", values = c("#FF930B", "#860000", "#FFFF6F", "#9F0AB1",
                    "#503287", "#7496BD", "#19b0fc", "#A9E2F3", "#92FE96", "#21b54b", "#D6FEB5", "#D0D0D0"))+
  theme_economist()+scale_colour_economist()+
...
plot_grid(HC.SE, plot_grid(NULL, p2, NULL, nrow = 3, rel_heights = c(1, 4.5)), rel_widths = c(6, 1))
ggsave("Coral x Genus by Treatment.tiff", width = 21, height = 10, units = "cm", dpi=1200, compression = "lzw")
