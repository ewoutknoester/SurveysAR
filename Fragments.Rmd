...
deaval.tot <- df2.surv.nd %>% group_by(Treatment) %>% tally()
colnames(deaval.tot)[2] <- "n.tot"

### Merge
deaval <- left_join(deaval, deaval.tot)
deaval$n.tot <- (deaval$n.tot/100)*deaval$Pct
deaval <- select(deaval, c("Treatment", "Cause.death", "n.tot", "Pct"))
colnames(deaval)[3] <- "n"
...
ggplot(deaval, aes(fill=Cause.death, y=Pct.tot, x=Treatment)) + 
  ...
  scale_fill_manual(values = c("#a4a2a8", "#c8d9e5", "#97bad2", "#7492A7", "#d8b4a0",
    ...
    scale_x_discrete(labels=c("BRU", "Cage", "Cake", "Comp"))+
   ...
   geom_text(data=deaval[(deaval$Cause.death=="Alive"),], aes(x=Treatment, y = Pct.tot - (1.8*se), 
...
ggsave("Fragments_Destiny.tiff", width = 21, height = 10, units = "cm", dpi=1200, compression = "lzw")
...
names(deaval.tot.genus) <- c("Genus", "Cause.death", "n")
deaval.tot.genus$Pct <- 0
