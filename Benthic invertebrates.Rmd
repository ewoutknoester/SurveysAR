...
df3.BI.snail <- subset(df2.selex, ((Treatment == "BRU" | Treatment == "Cage" | Treatment == "Cake" | Treatment == "Compound") &
                               Year == 2022 & (Group == "Corallivorous.snails"))  | 
                               ((Treatment == "Reference (-)" | Treatment == "Reference (+)") & Year >= 2020 & 
                               (Group == "Corallivorous.snails")))

### Total
df3.BI.snail.tot <- data_summary(df3.BI.snail, varname = "Density.m2", groupnames = c("Group", "Treatment", "Survey"))
df3.BI.snail.tot <- data_summary(df3.BI.snail.tot, varname = "sum", groupnames = c("Group", "Treatment"))
df3.BI.snail.tot <- select (df3.BI.snail.tot, -c(3, 5))
colnames(df3.BI.snail.tot)[which(names(df3.BI.snail.tot) == "sum")] <- "Density.m2.tot"
...
df3.BI.urchin <- data_summary(df3.BI.urchin, varname = "Density.m2", groupnames = c("Treatment","Group", "Genus", "Survey"))
df3.BI.urchin <- select (df3.BI.urchin, -c(6:9))
colnames(df3.BI.urchin)[which(names(df3.BI.urchin) == "sum")] <- "Density.m2"

### Total
df3.BI.urchin.tot <- data_summary(df3.BI.urchin, varname = "Density.m2", groupnames = c("Group", "Treatment", "Survey"))
df3.BI.urchin.tot <- data_summary(df3.BI.urchin.tot, varname = "sum", groupnames = c("Group", "Treatment"))
df3.BI.urchin.tot <- select (df3.BI.urchin.tot, -c(3, 5))
colnames(df3.BI.urchin.tot)[which(names(df3.BI.urchin.tot) == "sum")] <- "Density.m2.tot"
...
### Total
df3.BI.gastro.tot <- data_summary(df3.BI.gastro, varname = "Density.ha", groupnames = c("Group", "Treatment", "Survey"))
df3.BI.gastro.tot <- data_summary(df3.BI.gastro.tot, varname = "sum", groupnames = c("Group", "Treatment"))
df3.BI.gastro.tot <- select (df3.BI.gastro.tot, -c(3, 5))
colnames(df3.BI.gastro.tot)[which(names(df3.BI.gastro.tot) == "sum")] <- "Density.ha.tot"
...
df3.BI.gastro.tot.spec$Genus.o <- ifelse(df3.BI.gastro.tot.spec$Density.pct > 1, df3.BI.gastro.tot.spec$Genus, "Other")
df3.BI.gastro.tot.spec <- select (df3.BI.gastro.tot.spec, -c(2:4))
...
### Total
df3.BI.cucum.tot <- data_summary(df3.BI.cucum, varname = "Density.ha", groupnames = c("Group", "Treatment", "Survey"))
df3.BI.cucum.tot <- data_summary(df3.BI.cucum.tot, varname = "sum", groupnames = c("Group", "Treatment"))
df3.BI.cucum.tot <- select (df3.BI.cucum.tot, -c(3, 5))
colnames(df3.BI.cucum.tot)[which(names(df3.BI.cucum.tot) == "sum")] <- "Density.ha.tot"
...
### Total
df3.BI.star.tot <- data_summary(df3.BI.star, varname = "Density.ha", groupnames = c("Group", "Treatment", "Survey"))
df3.BI.star.tot <- data_summary(df3.BI.star.tot, varname = "sum", groupnames = c("Group", "Treatment"))
df3.BI.star.tot <- select (df3.BI.star.tot, -c(3, 5))
colnames(df3.BI.star.tot)[which(names(df3.BI.star.tot) == "sum")] <- "Density.ha.tot"
...
# Corallivorous snails by genus
  ggplot(df3.BI.snail.spec) +  
  geom_bar(position = "stack", stat = "identity", aes(x = Treatment, y = Density.m2, fill = forcats::fct_rev(Species)))+
  geom_errorbar(aes(x = Treatment, ymin=Density.m2.tot, ymax=Density.m2.tot+(1*se)), width=.2, size = 1, position=position_dodge(.9))+
  #geom_text(data = df3.HC.sum, aes(x=Treatment, y = Cover.tot + (1*se), label = siglet), 
 #           vjust= -0.8, hjust = 0.5, size = 5, fontface = "bold", position=position_dodge(.9))+
  geom_bar(data=df3.BI.snail.spec[(df3.BI.snail.spec$Species=="Drupella"),],
  ...
  labs(y = expression(paste("Corallivorous snails ( ", m^-2,")")), x = "Treatment", fill = "Genus")+
  guides(fill=guide_legend(title="Genus"))+
  scale_x_discrete(expand = c(0, 0.7), labels=c("BRU", "Cage", "Cake", "Comp", "Ref (-)", "Ref (+)"))+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.95))+
  theme(
...
# Gastropods by family
  ggplot(df3.BI.gastro.spec) +  
  geom_bar(position = "stack", stat = "identity", aes(x = Treatment, y = Density.ha, fill = Genus.o))+
  geom_errorbar(aes(x = Treatment, ymin=Density.ha.tot, ymax=Density.ha.tot+(1*se)), width=.2, size = 1, position=position_dodge(.9))+
  #geom_text(data = df3.HC.sum, aes(x=Treatment, y = Cover.tot + (1*se), label = siglet), 
 #           vjust= -0.8, hjust = 0.5, size = 5, fontface = "bold", position=position_dodge(.9))+
